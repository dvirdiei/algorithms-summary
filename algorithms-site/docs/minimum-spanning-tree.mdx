...
<section id="kruskal" className="greedyDoc__section container">
  ...
  <div style={{ 
    ...
    borderRadius: '12px',
    marginTop: '1rem',
    // הוספת Flexbox כדי להצמיד את כל הבלוק לשמאל
    display: 'flex',
    ...
    <pre className="greedyDoc__code" style={{
      ...
      margin: 0,
      direction: 'ltr',
      textAlign: 'left', // יישור מפורש של הטקסט בתוך ה-pre
      whiteSpace: 'pre'   // שומר על מבנה ההזחות המקורי
    }}>
{`Kruskal(G = (V, E), w):
    1.  A = ∅
    2.  for each v ∈ V:
...
<section id="prim" className="greedyDoc__section container">
  ...
  <div style={{ 
    ...
    <pre className="greedyDoc__code" style={{
      ...
      direction: 'ltr',
      textAlign: 'left',
      whiteSpace: 'pre' // שומר על ההזחות המדויקות של האלגוריתם
    }}>
{`Prim(G = (V, E), w, r):
  1.  for each u ∈ V:
  2.      key[u] = ∞
